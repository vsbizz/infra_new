    ...
    <div className="pt-20 md:pt-30 bg-white">
      ...
      <section className="relative overflow-hidden bg-white pt-10 md:pt-16 pb-16 md:pb-24">
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          ...
          {/* Breadcrumb — truncates long titles on mobile */}
          <nav className="flex items-center gap-1.5 md:gap-2 mb-8 md:mb-10 flex-wrap">
            ...
            <Link
              ...
              className="hidden sm:inline text-xs md:text-sm font-medium text-slate-400 hover:text-teal-600 transition-colors truncate max-w-[120px] md:max-w-none"
            >
              {parentService.title}
            </Link>
            <ChevronRight className="hidden sm:inline w-3 h-3 text-slate-300 shrink-0" />
            <span className="text-xs md:text-sm font-semibold text-slate-900 truncate max-w-[160px] sm:max-w-none">
              {subService.title}
            </span>
          </nav>
          ...
          <div className="grid lg:grid-cols-12 gap-10 md:gap-12 lg:gap-20 items-center">
            <div className="lg:col-span-7">
              ...
              <div className="space-y-4 md:space-y-6">
                ...
                <div className="text-base md:text-lg text-slate-600 leading-relaxed max-w-2xl">
      ...
      <section className="py-16 md:py-24 bg-slate-50/50 border-y border-slate-100">
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          <div className="max-w-3xl mb-12 md:mb-20">
            <h2 className="text-2xl sm:text-3xl md:text-5xl font-bold text-slate-900 tracking-tight mb-4 md:mb-6">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4">
            {subService.sections.map((item: any, idx: number) => (
              <div
                key={idx}
                className="bg-white p-6 md:p-8 rounded-2xl border border-slate-200/60 hover:border-teal-500/30 hover:shadow-xl hover:shadow-teal-900/5 transition-all duration-300 group"
              >
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          ...
          <div className="grid sm:grid-cols-2 md:grid-cols-3 gap-3 md:gap-4">
            {subService.deliverables.map((item: any, idx: number) => (
              <div
                key={idx}
                className="relative group p-6 md:p-8 bg-white border border-slate-200 rounded-2xl hover:border-teal-500/50 hover:shadow-2xl hover:shadow-teal-500/5 transition-all duration-300"
              >
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          <div className="grid lg:grid-cols-3 gap-10 md:gap-16">
            ...
            <div className="space-y-6 md:space-y-8">
              <div>
                <h2 className="text-2xl md:text-3xl font-extrabold leading-[1.1] tracking-tight text-slate-900 mb-3 md:mb-4">
              ...
              <a
                href={`/services/${serviceSlug}`}
                className="inline-flex items-center gap-3 bg-slate-50 text-slate-900 px-5 md:px-6 py-3.5 md:py-4 rounded-md font-bold text-sm hover:bg-slate-100 transition-colors border border-slate-100"
              >
            ...
            {/* Right: related sub-services grid — 1 col on mobile, 2 col on md+ */}
            <div className="lg:col-span-2 grid sm:grid-cols-2 gap-x-8 md:gap-x-12 gap-y-10 md:gap-y-16">
              {Object.entries(parentService.subServices)
                ...
                .map(([slug, data]: any) => (
                  <a
                    ...
                    className="group block border-t border-slate-100 pt-6 md:pt-8 transition-all"
                  >
                    <div className="flex items-start justify-between mb-3 md:mb-4 gap-2">
                      <h3 className="text-lg md:text-xl font-bold text-slate-900 group-hover:text-teal-600 transition-colors font-display leading-snug">
                        {data.title}
                      </h3>
                    ...
                    <p className="text-slate-500 text-sm md:text-base leading-relaxed line-clamp-3">
                      {data.headline || (data.description && data.description[0])}
                    </p>
                  </a>
                ))}
            </div>
